...
\end{figure}
The reason is that always there is some initial delay while running MapReduce jobs, for example checking input/output paths, split creation, map creation, management of the tasks etc. In this case sequential version is preferible.
...
\end{figure}
As we can see Hadoop has not good performance also this time. By default, in fact, EMR considers just 2 mappers if the input size is only one file. In addiction, time to manage and move data to the distributed environment is required.
...
\section{Conclusions}
In this paper I compared the sequential and distributed version of the Twitter sentimental analysis with Hadoop framework. For enough big data set, distributed version is preferible for the time spent and the amount of data processable in the same time. In same cases it achieved 9 of speedup with 6 nodes. Hadoop has dimostrated to be a valid solution to processing huge amount of data. 
